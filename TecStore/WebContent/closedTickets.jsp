...
Collection<?> collection = (Collection<?>) request.getAttribute("closedTicketsByUser");
int numberPage;
...
if (collection == null) {
	response.sendRedirect("TicketOperations?action=searchClosedTicketsByUser");
}
%>
...
<body>
	...
	<center>
		<h2>
			<i class="fas fa-question"></i> Ticket Chiusi
		</h2>
		<br>
		<h5>
			Benvenuto nei tuoi ticket chiusi! Qui potrai visualizzare tutti i
			ticket risolti, e dunque chiusi, con le relative informazioni.<br>
		</h5>
	</center>
	...
	<center>
		<table id="ticketTable" style="width: 88%">
			...
			<tr style="margin-bottom: 2%">
				<td><center><%=ticket.getCodiceOrdine()%></center></td>
				<td><center><%=ticket.getTitolo()%></center></td>
				<td><center>
						<button style="width: 70%" id="conferma"
							onclick="window.location.href=''">Conversazione</button>
					</center></td>
				<td><center><%=ticket.getCodiceOrdine()%></center></td>
				<td><center><%=ticket.getStato()%></center></td>
			<tr>
				<th>&nbsp;</th>
				<th>&nbsp;</th>
				<th>&nbsp;</th>
			...
			<div id="previousPag" class="pagingAllOrders">
				...
				%>
				<a
					href="TicketOperations?action=searchClosedTicketsByUser&numberPage=<%=numberPage - 1%>">Pagina
					Precedente</a>
			</div>
			<%
			}
			...
			%>
			<h1 class="noobject" style="text-align: center">Nessun Ticket
				Presente</h1>
			...
			<h1 class="noobject" style="text-align: center">Nessun Ticket
				Presente</h1>
			<a id="getback"
				href="TicketOperations?action=searchClosedTicketsByUser&numberPage=<%=numberPage - 1%>">
				<--- Torna indietro</a>
			<%
			}
			%>
		...
		<button style="width: 13%; cursor: pointer" id="cancel"
			onclick="window.location.href='supportTicket.jsp'">Torna
			indietro</button>
	</center>
